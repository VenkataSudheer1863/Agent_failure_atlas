...
def load_afad(path="dataset/afad_v1.jsonl"):
...
def fig_category_distribution(records):
    cat_counts = Counter(r["failure_label"] for r in records if r.get("failure_label"))
    ...
    ax.set_title("AFAD v1.0: Failure Category Distribution (N = 1,000)", fontweight="bold", pad=10)
...
def fig_model_comparison(records):
    model_data = {m: {"total": 0, "failures": 0, "severity": [], "recovered": 0}
                  for m in MODELS}
    for r in records:
        m = r.get("model")
    ...
    rec_rates   = [model_data[m]["recovered"] / max(model_data[m]["failures"], 1) * 100 for m in names]
    ...
    for ax, vals, title, xlabel, xlim, fmt in [
        (axes[0], fail_rates,  "Failure Rate (%)",      "Failure Rate (%)",  (40, 75), "{:.1f}%"),
        (axes[1], rec_rates,   "Recovery Rate (%)",     "Recovery Rate (%)", (0, 50),  "{:.1f}%"),
        (axes[2], severities,  "Mean Severity Score",   "Mean Severity (1–5)",(2.5, 4), "{:.2f}"),
    ]:
    ...
    fig.suptitle("Cross-Model Failure Analysis — AFA Benchmark (N = 1,000)",
...
def fig_subcategory_frequency(records):
    ...
    ax.set_title("Top 15 Failure Subcategories by Frequency", fontweight="bold")
...
def fig_task_model_heatmap(records):
    ...
    sns.heatmap(df, annot=True, fmt=".1f", cmap="RdYlGn_r", linewidths=0.4, ax=ax,
                vmin=35, vmax=85, cbar_kws={"label": "Strict Failure Rate (%)", "shrink": 0.8})
...
def fig_failure_prediction_auc():
    steps = [1, 2, 3, 5]
    lr_auc  = [0.505, 0.517, 0.504, 0.520]
    rf_auc  = [0.560, 0.552, 0.574, 0.533]
    lr_std  = [0.036, 0.039, 0.039, 0.038]
    rf_std  = [0.043, 0.018, 0.038, 0.034]
