...
Usage:
    ...
    python analysis/failure_prediction.py --results-dir experiments/results/raw
"""
...
def load_trajectories(results_dir: str = "experiments/results/raw") -> List[Dict]:
    records = []
...
def extract_early_features(record: Dict, n_early_steps: int = 3) -> Dict:
    ...
    model_map = {
        "Llama-3.1-8B": 0, "Llama-4-Scout-17B": 1, "Qwen3-32B": 2,
        "Llama-3.3-70B": 3, "GPT-OSS-20B": 4, "GPT-OSS-120B": 5
    }
...
def train_failure_predictor(records: List[Dict], output_dir: str = "analysis/results") -> None:
    ...
    results = {"auc_by_steps": {}, "feature_importances": {}}
    ...
    for n_steps in [1, 2, 3, 5]:
        ...
        results["auc_by_steps"][str(n_steps)] = {
            "lr_auc_mean": round(float(lr_scores.mean()), 4),
            "lr_auc_std": round(float(lr_scores.std()), 4),
    ...
    for feat, imp in importances:
        ...
        results["feature_importances"][feat] = round(float(imp), 4)

    # Save results to JSON
    out_path = Path(output_dir) / "failure_prediction_results.json"
